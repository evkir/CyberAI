...
class CyberAIConfig:
    ...
    # An empty authorized scope is a violation, not a warning: the exploit
    # phase refuses to run against a target nobody named. On by default since
    # 1.7.0 -- absence of authorisation is not authorisation, and the burden
    # of saying so belongs on the run that wants to proceed anyway, which
    # says it with --no-strict-scope.
